...
func (r *Resources) List(filterResource ...func(ezkube.ResourceId) bool) []ezkube.ResourceId {
	res := make(sortableSliceOfString, 0, len(r.set))
	m := make(map[string]ezkube.ResourceId, len(r.set))
	...
	for _, key := range res {
		...
		if !filtered {
			resources = append(resources, m[key])
		}
	}
	return resources
}
...
func (r *Resources) Insert(resources ...ezkube.ResourceId) {
	for _, resource := range resources {
		inserted := false
		for i, key := range r.set {
			if Key(key) == Key(resource){
				r.set[i] = resource
				inserted = true
				break
			}
		...
		// insertIndex := sort.Search(r.Length(),
		// 	func(i int) bool {
		// 		return r.sortFunc(resource, r.set[i])
		// 	})
		...
		// // checkIdx := insertIndex - 1

		// // if checkIdx >= 0 && r.compareFunc(resource, r.set[checkIdx]) == 0 {
		// // 	r.set[checkIdx] = resource
		// // 	continue
		// // }
		// // if insertIndex < len(r.set) && r.compareFunc(resource, r.set[insertIndex]) == 0 {
		// // 	r.set[insertIndex] = resource
...
func (r *Resources) Delete(items ...ezkube.ResourceId) {
	...
	for _, item := range items {
		for i, key := range r.set {
			if Key(key) == Key(item){
				r.set = append(r.set[:i], r.set[i+1:]...)
				break
			}
		}
	}
